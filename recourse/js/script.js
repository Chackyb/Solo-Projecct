$(window).on("load", function () {
    $("#status").fadeOut(1000);
    $("#preloader").delay(1100).
    fadeOut(1000);
});
// owl-carousel
$(document).ready(function () {
    $("#team-members").owlCarousel({
        loop: true,
        margin: 10,
        nav: true,
        responsive: {
            0: {
                items: 1
            },
            1000: {
                items: 2
            }
        }
    });
});
// progress bar
$(document).ready(function () {
    $("#progress-elements").waypoint(function () {
        $(".progress-bar").each(function () {
            $(this).animate({
                    width: $(this).attr("aria-valuenow") + "%",
                },
                3000
            )
        });
        this.destroy();
    }, {
        offset: "bottom-in-view",

    });
});
// Responsive js
$(document).ready(function () {
    $('#services-tab').responsiveTabs({
        animation: 'slide',
    });
    // active bar
    $('#services-tab ul li a').click(function () {
        var current_link = $(this);
        var current_link_href = current_link.attr('href');
        $("#services-tab ul li a").removeClass('active');
        current_link.addClass('active');
    });
});
// window on load isotope
$(window).on("load", function () {
    // init Isotope
    $('.isotope-container').isotope({
        // options
    });
    // filter items on button click
    $('#isotope-filter').on('click', 'button', function () {
        var filterValue = $(this).attr('data-filter');
        $('.isotope-container').isotope({
            filter: filterValue
        });
        // active button
        $('#isotope-filter').find(".active").removeClass("active");
        $(this).addClass("active");
    });

});
// magnifire js
$(document).ready(function () {
    $('#protfolio-wapper').magnificPopup({
        delegate: 'a', // child items selector, by clicking on it popup will open
        type: 'image',
        gallery: {
            enabled: true
        },
    });
});
// testimonials js
$(document).ready(function () {
    $("#testimonial-slider").owlCarousel({
        loop: true,
        margin: 10,
        nav: true,
        responsive: {
            0: {
                items: 1,
            },
            1000: {
                items: 1,
            },
        },
    });
});
// counterup js
$(document).ready(function () {
    $('.counter').counterUp({
        delay: 10,
        time: 1000
    });
});
// client js
$(".client-list").owlCarousel({
    loop: true,
    margin: 10,
    nav: true,
    responsive: {
        0: {
            items: 1,
        },
        600: {
            items: 3,
        },
        1000: {
            items: 5,
        },
    },
});
// Map Section
function initMap() {
    map = new google.maps.Map(document.getElementById("map"), {
        center: {
            lat: -34.397,
            lng: 150.644
        },
        zoom: 8,
    });
}
// nav Section
$(document).ready(function () {
    $(window).scroll(function () {
        // for show
        if ($(window).scrollTop() > 50) {
            $("nav").addClass("white-nav-top");
            $(".navbar-brand img").attr("src", "./recourse/img/logo/logo-dark.png");
        } else {
            $("nav").removeClass("white-nav-top");
            $(".navbar-brand img").attr("src", "./recourse/img/logo/logo.png");
        };
    });
});